    ...
    if request.method == 'POST':
        ...
        if not TUAB:
            ...
            for subjectOfEach in subsOfQuestions:
                bq = Question.objects.filter(comp__iexact=comp).filter(subject__in=subjectOfEach).order_by('?')
                # bq = Question.objects.get(comp__iexact=comp, subject__iexact=subjectOfEach)#.order_by('?')
                bonusQuestions.append(bq)
            return render(request, 'questionset.html', {'questions': questions, 'bonusQuestions': bq, 'includeBonuses': TUAB})
